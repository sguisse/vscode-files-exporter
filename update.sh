...
# ────────────────────────────────────────────────────────────────────────────────
# REFACTOR UI.CONTROLLER.JS: FIX SYNTAX ERROR AND IMPLEMENT RUN BUTTON TOOLTIP ICONS
# ────────────────────────────────────────────────────────────────────────────────
# To avoid any complex regex parsing or trailing comma collisions, we safely overwrite
# the ui.controller.js entirely with a clean object structure and the requested tooltip logic.

mkdir -p src/webview/js/core

cat << 'EOF' > src/webview/js/core/ui.controller.js
...
echo "✅ Script modified. SyntaxError fixed and dynamic 'Save' & 'Tuning' icons mapped natively onto the run button UI."
