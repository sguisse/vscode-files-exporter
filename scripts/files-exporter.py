...
def compile_regexes(name, patterns):
    ...
    try:
        if isinstance(patterns, str):
            # Robust splitting handling commas injected by the TS orchestrator and newlines
            patterns = re.split(r'[\n,;]', patterns)
...
class StreamExporter:
    ...
    def end(self, folders_count, ext_counts, size_rej, size_bounds, exc_counts):
        if self.fmt == 'json':
            self.out_file.write('\n  ]\n}\n')
        elif self.fmt == 'xml':
            self.out_file.write('  </files>\n</export>\n')
...
def open_new_chunk(index, ext=""):
    global DEST_DIR, TIMESTAMP, OUTPUT_FORMAT, GROUP_BY_EXT, GENERATED_FILES
...
def update_bounds(bounds_dict, ext, sz):
    bounds_dict[ext]['min'] = min(bounds_dict[ext]['min'], sz)
    bounds_dict[ext]['max'] = max(bounds_dict[ext]['max'], sz)


def scan_and_process(filters):
    global DIRS_TO_PARSE, MAX_SIZE_BYTES, MAX_OUTPUT_SIZE_BYTES, GROUP_BY_EXT, PROCESSED_MANIFEST
    exporters = {}
    PROCESSED_MANIFEST = []

    g_fold, g_exts, g_rej, g_exc = set(), defaultdict(int), defaultdict(int), defaultdict(int)
    g_bnd = defaultdict(lambda: {'min': float('inf'), 'max': 0})

    c_fold, c_exts, c_rej, c_exc = set(), defaultdict(int), defaultdict(int), defaultdict(int)
    c_bnd = defaultdict(lambda: {'min': float('inf'), 'max': 0})

    def get_exporter(ext):
        ext_key = ext if GROUP_BY_EXT else "default"
        if ext_key not in exporters:
            idx = 1
            out_file, exporter = open_new_chunk(idx, ext_key if GROUP_BY_EXT else "")
            exporters[ext_key] = {'file': out_file, 'exporter': exporter, 'idx': idx}
        return exporters[ext_key]
    ...
    for source_path in DIRS_TO_PARSE:
        if os.path.isfile(source_path):
            ...
            fname, ext_dot = os.path.splitext(file)
            ext = ext_dot.lstrip('.')
            try:
                ...
                if MAX_SIZE_BYTES > 0 and sz > MAX_SIZE_BYTES:
                    c_rej[ext] += 1
                    g_rej[ext] += 1
                    update_bounds(c_bnd, ext, sz)
                    update_bounds(g_bnd, ext, sz)
                    continue
            except OSError:
                continue
            if not is_file_allowed(rel_fp, file, filters):
                c_exc[ext] += 1
                g_exc[ext] += 1
                continue
            try:
                with open(source_path, "r", encoding="utf-8", errors="replace") as f:
                    content = f.read()
                exp_data = get_exporter(ext)
                exp_data['exporter'].write_file(fname, ext, abs_f, content, rel_fp)
                c_exts[ext] += 1
                c_fold.add(abs_f)
                g_exts[ext] += 1
                g_fold.add(abs_f)
                PROCESSED_MANIFEST.append(os.path.abspath(source_path))
            ...
            for root, dirs, files in os.walk(source_path):
                ...
                if rel_root == "./.":
                    rel_root = "."
                if filters['exc_paths']:
                    dirs[:] = [d for d in dirs if not matches_any(f"{rel_root}/{d}", filters['exc_paths'])]
                for file in files:
                    fp, rel_fp = os.path.join(root, file), f"{rel_root}/{file}"
                    fname, ext_dot = os.path.splitext(file)
                    ext = ext_dot.lstrip('.')
                    if MAX_SIZE_BYTES > 0:
                        try:
                    ...
                    if not is_file_allowed(rel_fp, file, filters):
                        ...
                        continue
                    exp_data = get_exporter(ext)
                    out_file, exporter = exp_data['file'], exp_data['exporter']
                    if MAX_OUTPUT_SIZE_BYTES > 0 and out_file.tell() >= MAX_OUTPUT_SIZE_BYTES:
                        exporter.end(len(c_fold), c_exts, c_rej, c_bnd, c_exc)
                        out_file.close()
                        exp_data['idx'] += 1
                        new_out_file, new_exporter = open_new_chunk(exp_data['idx'], ext if GROUP_BY_EXT else "")
                        exporters[ext if GROUP_BY_EXT else "default"]['file'] = new_out_file
                        exporters[ext if GROUP_BY_EXT else "default"]['exporter'] = new_exporter
                        out_file, exporter = new_out_file, new_exporter
                        c_fold.clear()
                        c_exts.clear()
                        c_rej.clear()
                        c_bnd.clear()
                    ...
                    try:
                        with open(fp, "r", encoding="utf-8", errors="replace") as f:
                            content = f.read()
                        exporter.write_file(fname, ext, abs_f, content, rel_fp)
                        c_exts[ext] += 1
                        c_fold.add(abs_f)
                        g_exts[ext] += 1
                        g_fold.add(abs_f)
    ...
    for exp_data in exporters.values():
        exp_data['exporter'].end(len(c_fold), c_exts, c_rej, c_bnd, c_exc)
        exp_data['file'].close()
        chunks += exp_data['idx']
    return len(g_fold), dict(g_exts), dict(g_rej), dict(g_bnd), dict(g_exc), chunks
...
def print_configuration(inc_paths, exc_paths, inc_ext, exc_ext):
...
def main():
    ...
    global DIRS_TO_PARSE, DEST_DIR, OUTPUT_FORMAT, MAX_SIZE_BYTES, MAX_OUTPUT_SIZE_BYTES
    global GROUP_BY_EXT, GENERATE_LOG_CONSOLE, GENERATE_LOG_FILE, GENERATE_TREE_VIEW, TIMESTAMP, LOG_FILE_HANDLE

    # ─── MULTI-SEPARATOR SOURCE SPLITTING ROUTINE ───
    split_srcs = []
    for src_item in args.src:
        if src_item:
            # Clean re-division if the HMI sends values condensed by comma, semicolon or newline
            for path_part in re.split(r'[\n,;]', src_item):
                # Cleaning up whitespace and eradicating residual apostrophes/quotes from the shell
                cleaned_part = path_part.strip().strip("'\"").strip()
                if cleaned_part:
                    split_srcs.append(cleaned_part)

    # Resolution in absolute paths of the final deduplicated and cleaned table
    ...
    print_configuration(args.inc_paths, args.exc_paths, args.inc_ext, args.exc_ext)

    log(f"\nStarting export (Format: {OUTPUT_FORMAT.upper()})", emoji="🚀")
    folders_cnt, ext_cnts, rej_cnts, g_bnd, exc_cnts, chunks = scan_and_process(filters)
