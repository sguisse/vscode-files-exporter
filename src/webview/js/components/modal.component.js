export const ModalComponent = {
    triggerGuardrailValidationFlow(pathsArray, onSuccess, onCancel) {
        ...
        backdrop.innerHTML = `<div style="background: #1e1e1e; color: #ccc; padding: 20px; border-radius: 6px; border: 1px solid var(--vscode-panel-border); min-width: 450px;">
    ...
    triggerValidationErrorModal(errorText) {
        const backdrop = document.createElement('div');
        backdrop.style.cssText = 'position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 999999; display: flex; align-items: center; justify-content: center;';
        backdrop.innerHTML = `<div style="background: #1e1e1e; color: #ccc; padding: 20px; border-radius: 6px; border: 1px solid var(--vscode-panel-border); min-width: 400px;">
